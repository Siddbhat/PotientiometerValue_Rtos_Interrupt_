...
int main(void)
{
  ...
  osThreadDef(Task02, Task02_init, osPriorityIdle, 0, 128);
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
	  	   ...
	  	   sprintf(msg,"%hu\r",readValue);
	  	   HAL_UART_Transmit(&huart2,msg, strlen(msg),1000);
	  	   arr[0] = readValue & 0xFF;
	  	   arr[1] = (readValue >> 8) & 0xFF;
	  	   HAL_UART_Transmit_IT(&huart5,arr, sizeof(arr));
	  	   HAL_ADC_Stop(&hadc1);
	  	   HAL_Delay(1000);
	  	   osDelay(1);
 }
